...
def modify_email_message(raw_email):
    """
    params: 
        string raw_email: The raw email from the S3 bucket

    returns: 
        string message: The modified email message
        string from_addr: The new sender email
        string to_addr: The address the email is being forwarded to
    ...
    return message, from_addr, to_addr
...
def forward_ses_email(s3_key):
    ...
    message, from_addr, to_addr = modify_email_message(raw_email)
